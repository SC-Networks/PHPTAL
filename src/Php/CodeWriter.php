...
class CodeWriter
{
    ...
    {
        if ($this->_xmldeclaration && $this->getOutputMode() !== \PhpTal\PHPTAL::HTML5) {
    ...
    public function doTemplateFile($functionName, \PhpTal\Dom\Element $treeGen)
    {
    ...
    public function quoteAttributeValue($value)
    {
        ...
        if ($this->getOutputMode() == \PhpTal\PHPTAL::HTML5 && !preg_match($unsafe_attr_regex, $value)) {
            return $value;
        } else {
            return '"'.$value.'"';
        }
